...
--region FileStream
---@class plus.FileStream : plus.Class
...
--region BinaryReader
---@class plus.BinaryReader : plus.Class
...
--region BinaryWriter
---@class plus.BinaryWriter : plus.Class
...
function BinaryWriter:WriteChar(c)
    assert(type(c) == "string" and c:len() == 1, "invalid argument.")
    self._stream:WriteByte(c:byte())
end
...
---写入一个字符串
---@param s string @字符串
---@param nullTerminate boolean @是否要以'\0'结尾
