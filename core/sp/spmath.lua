...
local function GetCircle2(x, y, ra, rb, a, n, angle, rot, m)
    ...
    return function()
        if i < n then
            local ang_a = rot + c * i
            local ang_b = angle
            local _x, _y = circle2(ra, rb, ang_a, ang_b, x, y)
            i = i + 1
            return _x, _y
        end
    end
...
local function Per3D(x, y, z, dist)
    local a0 = atan2(192, dist)
    local ax = atan2(x, dist + z)
    local ay = atan2(y, dist + z)
    x = 192 * ax / a0
    y = 192 * ay / a0
    return x, y
end
